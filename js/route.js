...
function CalculateRoute(mapPoint, pollPoint) {
    ...
    routeParams.directionsLengthUnits = esri.Units.METERS; // !!!warning!!!
	routeParams.directionsLanguage = "FR-fr";
	routeParams.directionsRestrictionAttributeName ="Walking, OnewayPieton";
...
function ShowRoute(solveResult) {
    ...
    if (divDirections) {
        ...
        tdTotalDistance.innerHTML = "Distance totale: " + FormatDistance(directions.totalLength, "m&egrave;tres");
        tdTotalDistance.align = "left";

        var tdTotalTime = document.createElement("td");
        tdTotalTime.className = "tdTotalTime";
        tdTotalTime.innerHTML = "Temps de parcours: " + FormatTime(directions.totalTime);
    ...
    if (divDirections) {
        ...
        for (var i in solveResult.routeResults[0].directions.features) {
            ...
            var miles = FormatDistance(feature.attributes.length, "m&egrave;tres");
            ...
            if (i == 0) {
                ...
                if (selectedFeature.length > 0) {
                    td.innerHTML = feature.attributes.text.replace('Location 2', selectedFeature[0].attributes.NAME);
                }
                else {
                ...
                if (miles) {
                    td.innerHTML = feature.attributes.text + " (" + FormatDistance(feature.attributes.length, "m&egrave;tres") + ")";
                }
                else {
...
function FormatTime(time) {
    ...
    return hr + " heure(s) " + min + " minute(s)";
}